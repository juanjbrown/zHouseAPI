module.exports = function(schedulesParam) {
  ...
  function authenticate(req, res, next) {
    ...
    GLOBAL.mysqlGlobal.authenticate(req.headers.username, req.headers.apikey, function(status, response) {
      if(status === 403) {
    ...
    GLOBAL.mysqlGlobal.getNodes(function(status, response) {
      if(response.status === 'success') {
        for(var i=0;i<response.data.length;i++) {
          response.data[i].node = GLOBAL.zwaveGlobal.nodes[response.data[i].node_id];
        }
      }
      res.status(status).json(response);
    });
  ...
  router.post('/users', function(req, res) {
    GLOBAL.mysqlGlobal.getUser(req.headers.username, function(status, response) {
      if(response.data.role < 1) {
        GLOBAL.mysqlGlobal.createUser(req.body, function(status, response) {
          res.status(status).json(response);
        });
      } else {
        res.status(403).json({
  ...
  router.get('/users', function(req, res) {
    GLOBAL.mysqlGlobal.getUser(req.headers.username, function(status, response) {
      if(response.data.role < 1) {
        GLOBAL.mysqlGlobal.getUsers(function(status, response) {
          res.status(status).json(response);
        });
      } else {
        res.status(403).json({
